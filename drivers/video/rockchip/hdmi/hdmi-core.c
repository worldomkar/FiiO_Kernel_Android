...
struct delayed_work *hdmi_submit_work(struct hdmi *hdmi, int event, int delay, void *data)
{
	...
	hdmi_dbg("%s event %04x delay %d", __FUNCTION__, event, delay);
...
{
	hdmi_dbg("%s mute %d", __FUNCTION__, mute);
	if(hdmi->ops->setMute)
...
static void hdmi_work_queue(struct work_struct *work)
{
	...
	switch(event) {
		...
		case HDMI_HPD_CHANGE:
			...
			hdmi_dbg("hdmi_work_queue() - hpd is %d hotplug is %d", hpd, hdmi->hotplug);
			if(hpd != hdmi->hotplug) {
...
struct hdmi *hdmi_register(struct hdmi_property *property, struct hdmi_ops *ops)
{
	...
	hdmi_dbg("hdmi_register() - video source %d display %d",
	...
	if(hdmi->id == 0)
		...
		hdmi->switch_hdmi.name = kzalloc(32, GFP_KERNEL);
		memset((char *)hdmi->switch_hdmi.name, 0, 32);
		sprintf((char *)hdmi->switch_hdmi.name, "hdmi%d", hdmi->id);
	}
